...
class Animator:
    ...
    def lerp(self, object, destination, time, framerate):
        x_step,y_step = self.getLerpValues(object.coordinates,destination, time, framerate)
    ...
    def getLerpValues(self, pointA, pointB, time, framerate):
        
        x_delta = pointB[0]-pointA[0]
        y_delta = pointB[1]-pointA[1]

        x_speed = x_delta / time / framerate
        y_speed = y_delta / time / framerate
